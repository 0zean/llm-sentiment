...
def navbar():
    cond = rx.cond(State.score >= 0, c1='green.500', c2='red.500')
    return rx.box(
        rx.hstack(
            rx.hstack(
                rx.avatar(src='/disco-ball.png', fallback="M", variant="soft"),
                rx.heading("LLM Sentiment"),
                rx.desktop_only(
                    rx.badge(
                    State.current_chat,
            ...
            rx.hstack(
                rx.chakra.text("Sentiment: ", color='gray.500'),
                rx.chakra.text(f"{State.score}", color=cond)
            ),
            rx.hstack(
